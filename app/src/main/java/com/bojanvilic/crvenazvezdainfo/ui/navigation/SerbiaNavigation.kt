...
fun NavGraphBuilder.serbiaScreen(navController: NavHostController) {
    composable(TopLevelDestinations.Serbia.route) {
        val articlesViewModel: ArticlesViewModel = hiltViewModel()
        articlesViewModel.setCurrentCategory(Category.SERBIA.categoryIntValue)
        ArticlesScreen(
